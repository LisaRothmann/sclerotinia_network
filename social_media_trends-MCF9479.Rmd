...
scl.sm <- scl.sm %>% 
  mutate(asdate = case_when(
                            ...
                            Date == "11 November 2024" ~ as.Date("11/11/2024", format='%m/%d/%Y'),
                            Date == "01 March 2025" ~ as.Date("03/01/2025", format = '%m/%d/%Y'),
                            Date == "13 June 2025" ~ as.Date("06/13/2025", format = '%m/%d/%Y'),
                            Date == "30 August 2025" ~ as.Date("08/30/2025", format = '%m/%d/%Y'),
                            Date == "14 November 2025" ~ as.Date("11/14/2025", format = '%m/%d/%Y')))
```
...
ggplot +
  ...
  scl.sm %>%
  ggplot(aes(x = asdate, y = Followers, color = Platform)) +
  geom_line(aes(group = Platform), size = 1.2) +  # solid line for clarity
  geom_point(size = 3, shape = 21, fill = "white", stroke = 1.2) +  # outlined points
  scale_color_manual(values = c(
    "Facebook" = "#4E79A7",
    "Instagram" = "#962fbf",
    "X - formerly Twitter" = "#000000"
  )) +
  scale_x_date(
  ...
  ggsave("logo/vis/sm_trends_b.jpeg", units = "cm", width = 30, height = 15)
